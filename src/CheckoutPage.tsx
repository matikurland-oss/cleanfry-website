          ...
          <div className="lg:col-span-7 space-y-6 text-right">
            ...
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 text-right">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <input type="email" placeholder="אימייל *" value={email} onChange={(e) => { setEmail(e.target.value); setShowPayment(false); }} className="md:col-span-2 p-4 bg-slate-50 rounded-2xl border-none outline-none focus:ring-2 focus:ring-blue-500 transition-all text-right" />
            ...
            {/* קופון ותשלום */}
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 text-right">
              <h2 className="text-xl font-black mb-4 text-slate-800">הנחה</h2>
              <div className="flex gap-2 mb-6">
                <input type="text" placeholder="קוד קופון" value={coupon} onChange={(e) => { setCoupon(e.target.value); setShowPayment(false); }} disabled={isCouponApplied} className="flex-1 p-4 bg-slate-50 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 text-right" />
                {!isCouponApplied ? (
                  <button onClick={handleApplyCoupon} className="bg-slate-800 text-white px-6 rounded-2xl font-bold">החל</button>
                ) : (
                  <button onClick={handleRemoveCoupon} className="bg-red-50 text-red-500 px-6 rounded-2xl font-bold"><X size={18} /></button>
                )}
              </div>
          ...
          <div className="lg:col-span-5">
            <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100 sticky top-8">
              ...
              <div className="space-y-4">
                <div className="flex justify-between"><span>סה"כ ביניים:</span><span className="font-bold">₪{subtotal}</span></div>
                <div className="flex justify-between"><span>משלוח:</span><span className="font-bold">₪{currentShipping}</span></div>
                {isCouponApplied && <div className="flex justify-between text-green-600"><span>הנחה:</span><span className="font-bold">-₪{discount}</span></div>}
                <div className="border-t pt-4 flex justify-between text-2xl font-black"><span>סה"כ:</span><span>₪{totalPrice}</span></div>
              </div>
            </div>
          </div>
        </div>
